...
# Meteorites over the Year with Name Type

```{r meteorites over the year by name type}
...
# Meteorites over the Year with Fall

```{r meteorites over the year with fall}
meteorites_rm<-remove_missing(meteorites)

world <- ggplot() + theme_map()+
         borders("world", colour = "#353535", fill = "#74e04a") 

plot2<-world+geom_point(data=meteorites_rm,x=meteorites_rm$long,y=meteorites_rm$lat,
                 color=meteorites_rm$fall)+
             transition_states(meteorites_rm$year,transition_length = 1,state_length = 1)+
             shadow_mark(past = TRUE)+
             theme(plot.title = element_text(color = "black", 
                                           size = 15, 
                                           face = "bold"),
                   panel.background = element_rect(fill = "#5e8bed")) +
             ggtitle("Meteors Falling from the sky by Fall",subtitle = "Year:{closest_state}")

animate(plot2,nframes=253,fps=1)
```


# Meteorites and Classes 

```{r meteorites and classes}
test<-meteorites%>%
...
```{r meteorites and classes jitter plot}
test %>%
...
# Meteorites and their Mass by Class

```{r meteorites and masses by class}
test1<-meteorites %>%
            select(year,mass) %>%
            group_by(year) %>%
