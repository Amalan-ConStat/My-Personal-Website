...
knitr::opts_chunk$set(echo = TRUE,message=FALSE,warning=TRUE,fig.height = 12,fig.width = 12)
```
...
5 data-sets are given here, but I only will be discussing 3. They are Fluid Milk Sales, State Milk
...
Using [usmap]() package I am going to plot it into their respective 
...
```{r State Milk production Us Map,fig.height=14,fig.width=14}
...
This is similar to the next half years which is from 1994 to 2017 as well. Similar increase
...
```{r State Milk production Us Map 1,fig.height=14,fig.width=14}
...
strong. There are some states which have not produced more amount each year their 
previous years. 

The states Wyoming, Rhode island, Hawaii, Delaware and Alaska have very low amount of
milk production over the years.

```{r State Milk production over the years for states}
...
In order to do this I have used the [dplyr]() package and created a function
...
```{r State milk production altering by summation or mean}
...
```{r State milk production by region for summation}
...
in the Northeast region. Clear and highest variation is in for Pacific, Mountain and Lake States region.

```{r Summation by region for jitter}
...
```{r Summation by region for jitter by year animation}
...
```{r State milk production by region for average}
...
```{r average by region for jitter}
...
```{r average by region for jitter by year animation}
...
Products with the colors respectively red, yellow, blue and green.
...
colors respectively red, yellow, blue, green and white. Clearly Cheddar and mozzarella type cheese are
