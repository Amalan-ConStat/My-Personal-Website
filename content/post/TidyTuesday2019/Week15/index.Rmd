...
```{r decade of birth vs first title won}
player_dob %>% 
   remove_missing() %>%
...
```{r grand slams , Decade of Birth and age}
player_dob %>% 
  remove_missing() %>%
  mutate(grand_slam=recode_factor(grand_slam,
                                  'Wimbledon'="Wimbledon",
...
```{r gender vs grand slam with name}
grand_slams %>%
...
```{r gender vs grand slam with year}
p<-grand_slams %>%
...
```{r Top 10 Tennis player and outcomes}
top10<-grand_slams %>%
       group_by(name) %>%
       count(sort = TRUE) %>%
       head(10) %>%
...
```{r Top 10 Tennis Players and outome}
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
...
```{r Top 10 Tennis Players Winning}
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
      subset(outcome=="Won") %>%
...
```{r Top 10 Tennis Players Finalist}
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
...
```{r Top 10 Tennis Players Semi-Finalist}
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
...
```{r Top 10 Tennis Players Retired}
grand_slam_timeline %>%
      rename(name="player") %>%
      inner_join(top10,"name") %>%
