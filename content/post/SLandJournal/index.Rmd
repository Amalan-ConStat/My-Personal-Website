...
p<-subset(sjr_journals,country=="Sri Lanka") %>%
   ...
   ggplot(.,aes(x=str_wrap(factor(title),30),y=rank,color=year))+
         ...
         ggtitle("Ranks Changing over time for Sri Lankan Journals from 1996 to 2017",
                 subtitle = "Year : {round(frame_time)}")

animate(p,fps=1,nframes=21)  
```
...
p<-subset(sjr_journals,country=="Sri Lanka") %>%
   ...
   ggplot(.,aes(x=str_wrap(factor(title),30),y=sjr,color=year,shape=sjr_best_quartile))+
         ...
         ggtitle("SJR values Changing over time for Sri Lankan Journals from 1996 to 2017",
                 subtitle = "Year : {round(frame_time)}")+
...
p<-subset(sjr_journals,country=="Sri Lanka") %>%
   ...
   ggplot(.,aes(x=str_wrap(factor(title),30),y=ref_doc,color=year))+
         ...
         ggtitle("References per Document Changing over time for Sri Lankan Journals from 1996 to 2017",
                 subtitle = "Year : {round(frame_time)}")
