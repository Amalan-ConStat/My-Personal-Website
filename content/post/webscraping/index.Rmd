...
Below figure is the layout from the worldcup 1975 page. Also the highlighted area is chosen by me 
...
```{r Code for extracting html information,echo=FALSE}
# Load the packages
  library(rvest)
  library(stringr)
...
```{r One row information,echo=FALSE}
# one row information
  data_mat[4,]
```
...
```{r transforming the data and replacing notations,echo=FALSE}
...
```{r Selecting only few columns and spliting the one column,echo=FALSE}
...
Now we have a much better data frame than earlier, which makes alot sense. 
...
```{r Much more complex data extraction}
...
# acquiring who vs who and locations
  ...
  Location <- list(WhovsWhos$Column_05_2)
