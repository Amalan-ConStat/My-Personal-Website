---
title: 'Benchmarking optimization functions in R : Estimating the shape parameters
  by Maximizing the Log Likelihood value of Beta-Binomial Distribution.'
author: M.Amalan
date: '2018-12-04'
categories:
  - fitODBOD
...
Above mentioned Beta-Binomial distributions Probability Mass Function(PMF) is denoted as
...
Time to complete optimization and Analytical Methods used with the assistance of two tables. Alcohol Consumption data 
...
shape parameter values which we want to estimate. 

Here the shape parameters a and b are in the region of greater than zero but less than positive infinity($+\infty >a,b>0$). 
...
estimation was done for shape parameter values a and b using the ucminf function. Below are the initial understanding of
the ucminf function 
...
It should be noted that in all 5 previous functions we only outpus in the form of lists, but
...
Because as I said in the earlier table the estimated values are slightly different but does not make any strong influence 
on the results.
...
compare p-values and over-dispersion and understand if using different optmization functions had
any effect on them.
